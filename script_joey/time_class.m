function [fasce_h,fasce_q] = time_class(year,sd)
%TIME_CLASS produces a file and 2 variables in workspace with time classification 
% according to AEEGSI (now ARERA) deliberation no. 181/06
% INPUT
%       year:   
...
%               
% OUTPUT
%       output is time_class_'year'.txt
...
%  ========================
%         Mon - Fri
% F1:   fr 08.00   to 19.00
% F2:   fr 07.00   to 08.00
% F2:   fr 19.00   to 23.00
% F3:   fr 00.00   to 07.00
% F3:   fr 23.00   to 24.00
%
%            Sat
% F2:   fr 07.00   to 23.00
% F3:   fr 00.00   to 07.00
% F3:   fr 23.00   to 24.00
%
%            Sun
% F3:   fr 00.00   to 24.00
% =========================
% 
% Example:
% 
% year = 2019;
% sd = ['01/01/2019';'06/01/2019';'22/04/2019';'25/04/2019';'01/05/2019';'02/06/2019';'15/08/2019';'01/11/2019';'08/12/2019';'25/12/2019';'26/12/2019'];
...
for h=1:12, % cycle on months
    ...
    for i = 1:size(wddays,1),
        % F1:   fr 08.00   to 19.00
        fasce(((wddays(i)-1)*24+1)+8:((wddays(i)-1)*24+1)+18,1) = 1;
        % F2:   fr 07.00   to 08.00
        % F2:   fr 19.00   to 23.00
        fasce(((wddays(i)-1)*24+1)+7,1) = 2;
        fasce(((wddays(i)-1)*24+1)+19:((wddays(i)-1)*24+1)+22,1) = 2;
        % F3:   fr 00.00   to 07.00
        % F3:   fr 23.00   to 24.00
        fasce(((wddays(i)-1)*24+1)+23,1) = 3;
        fasce(((wddays(i)-1)*24+1):((wddays(i)-1)*24+1)+6,1) = 3;
    end
    calsats = cal2(:,end);
    sats = sort(calsats(find(calsats ~= 0)));
    for i = 1:size(sats,1),
        % F2:   fr 07.00   to 23.00
        fasce(((sats(i)-1)*24+1)+7:((sats(i)-1)*24+1)+22,1) = 2;
        % F3:   fr 00.00   to 07.00
        % F3:   fr 23.00   to 24.00
        fasce(((sats(i)-1)*24+1)+23,1) = 3;
        fasce(((sats(i)-1)*24+1):((sats(i)-1)*24+1)+6,1) = 3;
    end
    calsuns = cal2(:,1);
    suns = sort(calsuns(find(calsuns ~= 0)));
    for i = 1:size(suns,1),
        % F3:   fr 00.00   to 24.00
        fasce(((suns(i)-1)*24+1):((suns(i)-1)*24+1)+23,1) = 3;
    end

    % Modification for special holidays in current month
    if ~isempty(find(sdm == h))
        idx = find(sdm == h);
        for i = 1:length(idx),
            fasce(((sdd(idx(i))-1)*24+1):((sdd(idx(i))-1)*24+1)+23,1) = 3;
        end
    end
...
save(strcat('time_class_',num2str(year)),'fasce_h','fasce_q');
...
fid = fopen(['time_class_q_',num2str(year),'.txt'],'w+');
...
fid = fopen(['time_class_h_',num2str(year),'.txt'],'w+');
